...
try:
    ...
    # COD-VAE vendors the same stochastic-depth primitive. In eval mode this
    # path is an identity, so the fallback does not change benchmark semantics.
    from cod.models.vae.modules.timm_layers import DropPath
...
class AutoEncoder(nn.Module):
    ...
    def encode(self, pc):
        ...
        ###### fps
        if torch_cluster_fps is None:
            sampled_pc = pointops.fps(pc, self.num_latents)
        else:
            flattened = pc.view(B*N, D)
            batch = torch.arange(B, device=pc.device)
            batch = torch.repeat_interleave(batch, N)
            ratio = 1.0 * self.num_latents / self.num_inputs
            idx = torch_cluster_fps(flattened, batch, ratio=ratio)
